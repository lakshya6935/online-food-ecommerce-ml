...
delay_model = joblib.load("delivery_delay_model.pkl")
rating_model = joblib.load("service_rating_model.pkl")
...
minutes = st.number_input("Delivery Time (Minutes)", min_value=1, max_value=200, value=35)

if st.button("Predict Delay"):
    result = delay_model.predict(pd.DataFrame({
        "Delivery Time (Minutes)": [minutes]
    }))[0]
    st.success("Delivery Delay: " + str(result))

st.header("2. Service Rating Prediction")
feedback = st.selectbox("Customer Feedback", [
...
if st.button("Predict Rating"):
    result = rating_model.predict(pd.DataFrame({
